...
- [x] A `citekick` CLI exists that accepts a query string.
...
- [x] Agent configuration: Guide users to configure API keys in `~/.config/citekick/config.toml` and integrate the `citekick` MCP server into agent environments (e.g., opencode, Claude Desktop).

### Technical Notes and Implementation Hints

- Core Files: `src/citekick/config.py` (user TOML loading and import-time constants), `src/citekick/__main__.py` (orchestrator), `src/citekick/paper.py` (Paper record), `src/citekick/sources.py` (Source enum + strategy registry), `src/citekick/sources/` (one strategy module per source: `semantic_scholar.py`, `arxiv.py`, `pubmed.py`, `openalex.py`, `crossref.py`, `europepmc.py`, `openreview.py`, `dblp.py`), `src/citekick/output.py` (JSON + Markdown rendering), `src/citekick/mcp_server.py` (FastMCP `search_papers` tool), `src/citekick/cli.py`
- Tests should include unit tests (fast interface tests with mocks) and integration tests (make real api calls)
- Dependencies: `mcp` (MCP server framework), `httpx` (async HTTP client), `tenacity` (retry/backoff). The `citekick` and `citekick-mcp` entry points are defined in `pyproject.toml` `[project.scripts]` and exposed as `hatch run` scripts.
